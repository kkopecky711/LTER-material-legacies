...
model_outputs_clean <- model_outputs %>%
  ...
  mutate(
    across(
      ...
      .fns = ~ ifelse(abs(.) < 0.001, formatC(., format = "e", digits = 2), formatC(., format = "f", digits = 3))
    )
  )
...
# STEP 5: Reorder ecosystems to match the forest plot
ecosystem_order <- c(
