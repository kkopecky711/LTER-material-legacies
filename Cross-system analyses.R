...
# Read in effect size datafiles
mcr_effect  <- read_csv("Standardized/mcr_effect.hybrid_2sdX_1sdY.csv")
bnz_effect  <- read_csv("Standardized/bnz_effect.hybrid_2sdX_1sdY.csv")
vcr_effect  <- read_csv("Standardized/vcr_effect.hybrid_2sdX_1sdY.csv")
gce_effect  <- read_csv("Standardized/gce_effect.z.csv")          # (kept as z if no hybrid)
luq_effect  <- read_csv("Standardized/luq_effect.z.csv")          # (kept as z if no hybrid)
fce_effect  <- read_csv("Standardized/fce_effect.hybrid_2sdX_1sdY.csv")
songs_effect<- read_csv("Standardized/songs_effect.hybrid_2sdX_NB.csv") # NB: estimate is log-rate ratio
hfr_effect  <- read_csv("Standardized/hfr_effect.z.csv")          # (kept as z if no hybrid)
knz_effect  <- read_csv("Standardized/knz_effect.z.csv")          # (kept as z if no hybrid)
hja_effect  <- read_csv("Standardized/hja_effect.hybrid_2sdX_1sdY.csv")
...
effect_sizes <- effect_sizes %>% 
  mutate(
    ecosystem = c("Coral reef", "Boreal forest",  "Oyster reef", "Salt marsh", "Tropical forest",
                  "Mangrove forest", "Kelp forest", "Temperate forest", "Tallgrass prairie", "Temperate rainforest"),
    significance = ifelse(p.value < 0.05, "Significant", "Not Significant")
  )
...
effect_sizes <- effect_sizes %>%
  mutate(
    system = case_when(
      ecosystem %in% c("Coral reef", "Kelp forest", "Oyster reef", "Salt marsh", "Mangrove forest") ~ "Marine",
      TRUE ~ "Terrestrial"
    ),
    alpha_level = ifelse(significance == "Significant", 1, 0.4)
  )
...
## Forest plot with system type and transparency based on significance 
ggplot(effect_sizes, aes(x = estimate, y = reorder(ecosystem, estimate), color = system)) +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high, alpha = alpha_level),
...
# Add variables for absolute values of effect sizes and CIs
effect_sizes <- effect_sizes %>% 
  mutate(
    estimate.abs  = abs(estimate),
    conf_low.abs  = if_else(estimate < 0, abs(conf.low), conf.low),
    conf_high.abs = abs(conf.high)
  )

# Plot absolute values
ggplot(effect_sizes, aes(x = estimate.abs, y = reorder(ecosystem, estimate.abs), color = system)) +
  geom_errorbarh(aes(xmin = conf_low.abs, xmax = conf_high.abs, alpha = alpha_level),
                 height = 0, linewidth = 1) +  
  geom_point(size = 3, color = "black") +
  geom_point(size = 2) +
  labs(x = "Effect size (± 95% CI), abs. value",
