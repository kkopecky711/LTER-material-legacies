...
```{r}
# Load burn year data and filter for only four-year watershed
burns.four_year <- read_csv("Data/Konza Prairie/KFH011.csv") %>% 
  clean_names() %>% 
  filter(watershed == "4B",
         code != "004A") %>% 
  select(year) %>% 
...
Merge burn years and biomass data
```{r}
# Merge burn data with biomass data
knz.four_year <- knz.four_year %>% 
  left_join(burn_data_filled, by = "year") 
...
# Manually specify the last known burn year before biomass data starts
#last_known_burn <- 1991  

# knz.four_year <- knz.four_year %>% 
#   arrange(year) %>%
#   mutate(last_burn = ifelse(burn_year == "yes", year, NA)) %>%
#   fill(last_burn, .direction = "down") %>%  # Fill last known burn year down
#   mutate(last_burn = ifelse(is.na(last_burn), last_known_burn, last_burn),  # Replace NAs with 1991
#          years_since_burn = year - last_burn) %>%  # Calculate years since last burn
#   filter(lvgrass > 0,
#          pryrdead > 0)
...
predictions.knz <- ggpredict(knz.glmm, terms = ~ pryrdead*watershed)
...
# glmm of live grass biomass ~ previous year dead biomass*watershed
knz.glmm <- glmmTMB(lvgrass ~ pryrdead*watershed + (1|transect),
                    data = knz)
summary(knz.glmm)
...
vcr <- read_csv("Data/Virginia Coastal/Oyster_Count_Data_2022_03_10.csv") %>% 
  ...
  mutate(year = substr(date, nchar(date) - 1, nchar(date)),
         ...
         season = case_when( # Assign seasons based on month
          ...
          TRUE ~ NA_character_)) %>% 
  select(year, season, site, sample, species, species_count)
...
Meeting with Bob and Kyle:
- Try maintaining quadrats as replicates (nested within transect and site), no time lag
- Check LTE data for juvenile kelp ~ dead holdfasts (percent cover? presence/absence?)
- Look into why dead holdfasts start only in 2008
...
sbc.macro_dmh.summary %>%
  ggplot()+
  geom_ribbon(data = sbc.macro_dmh.predictions, 
              aes(x = x, y = predicted, ymin =conf.low, ymax = conf.high), 
              alpha =0.5,
              fill = "#3B4F8E")+
  geom_line(data = sbc.macro_dmh.predictions, 
            aes(x = x, y = predicted))+
  geom_jitter(aes(x = dmh_cover.mean, y = macro_cover.mean),
             alpha = 0.6,
             size = 2)+
  #scale_y_continuous(limits = c(0,14)) +
  labs(x = "% cover of dead holdfasts",
       y = "% cover of live Macrocystis") +
  theme_classic(base_size = 12)
...
```{r}
# glmm of canopy cover ~ dead holdfast cover
sbc.canopy_dmh.glmm <- glmmTMB(canopy_cover.mean ~ dmh_cover.mean + (1|site) + (1|year),
                              family = gaussian(link = "identity"),
                              data = sbc.canopy_dmh.summary)
summary(sbc.canopy_dmh.glmm)

# Check residuals
hist(residuals(sbc.canopy_dmh.glmm))
car::qqPlot(residuals(sbc.canopy_dmh.glmm))

# Plot model predictions and create dataframe 
plot(ggpredict(sbc.canopy_dmh.glmm, terms = ~dmh_cover.mean))
sbc.canopy_dmh.predictions <- as.data.frame(ggpredict(sbc.canopy_dmh.glmm, terms = ~dmh_cover.mean))
...
sbc.canopy_dmh.summary %>%
  ggplot()+
  geom_ribbon(data = sbc.canopy_dmh.predictions, 
              aes(x = x, y = predicted, ymin =conf.low, ymax = conf.high), 
              alpha =0.5,
              fill = "#3B4F8E")+
  geom_line(data = sbc.canopy_dmh.predictions, 
            aes(x = x, y = predicted))+
  geom_jitter(aes(x = dmh_cover.mean, y = canopy_cover.mean),
             alpha = 0.6,
             size = 2) +
  scale_y_continuous(limits = c(0,6)) +
  labs(x = "% cover of dead holdfasts",
       y = "% cover of canopy-forming spp.") +
  theme_classic(base_size = 12)
```
...
```{r}
# glmm of macro cover ~ dead holdfast cover
sbc.under_dmh.glmm <- glmmTMB(under_cover.mean ~ dmh_cover.mean + (1|site) + (1|year),
                              family = gaussian(link = "identity"),
                              data = sbc.under_dmh.summary)
summary(sbc.under_dmh.glmm)

# Check residuals
hist(residuals(sbc.under_dmh.glmm))
car::qqPlot(residuals(sbc.under_dmh.glmm))

# Plot model predictions and create dataframe 
plot(ggpredict(sbc.under_dmh.glmm, terms = ~dmh_cover.mean))
sbc.under_dmh.predictions <- as.data.frame(ggpredict(sbc.under_dmh.glmm, terms = ~dmh_cover.mean))
...
sbc.under_dmh.summary %>%
  ggplot()+
  geom_ribbon(data = sbc.under_dmh.predictions, 
              aes(x = x, y = predicted, ymin =conf.low, ymax = conf.high), 
              alpha =0.5,
              fill = "#3B4F8E")+
  geom_line(data = sbc.under_dmh.predictions, 
            aes(x = x, y = predicted))+
  geom_jitter(aes(x = dmh_cover.mean, y = under_cover.mean),
             alpha = 0.6,
             size = 2)+
  labs(x = "% cover of dead holdfasts",
       y = "% cover of understory algal spp.") +
  theme_classic(base_size = 12)
```
...
# Merge dead holdfast data and live macro data and remove quads with 0% dead holdfast cover
sbc.algae_dmh <- merge(sbc_algae, sbc_dead)
sbc.algae_dmh <- sbc.algae_dmh %>% 
  filter(dmh_cover > 0)

ggplot(sbc.algae_dmh, aes(x = dmh_cover, y = percent_cover)) +
  geom_jitter()

# summarize by year & site; calculate normalized values
sbc.algae_dmh.summary <- sbc.algae_dmh %>% 
  group_by(site, year) %>% 
  summarize(algae_cover.mean = mean(percent_cover),
            dmh_cover.mean = mean(dmh_cover)) %>% 
  mutate(percent_cover.norm = algae_cover.mean/max(algae_cover.mean),
         dmh_cover.norm = dmh_cover.mean/max(dmh_cover.mean))
...
```{r}
# glmm of macro cover ~ dead holdfast cover
sbc.algae_dmh.glmm <- glmmTMB(algae_cover.mean ~ dmh_cover.mean + (1|site) + (1|year),
                              family = gaussian(link = "identity"),
                              data = sbc.algae_dmh.summary)
summary(sbc.algae_dmh.glmm)

# Check residuals
hist(residuals(sbc.algae_dmh.glmm))
car::qqPlot(residuals(sbc.algae_dmh.glmm))

# Plot model predictions and create dataframe 
plot(ggpredict(sbc.algae_dmh.glmm, terms = ~dmh_cover.mean))
sbc.algae_dmh.predictions <- as.data.frame(ggpredict(sbc.algae_dmh.glmm, terms = ~dmh_cover.mean))

```

Visualization of normalized model
```{r}
sbc.algae_dmh.summary %>%
  ggplot()+
  geom_ribbon(data = sbc.algae_dmh.predictions, 
              aes(x = x, y = predicted, ymin =conf.low, ymax = conf.high), 
              alpha =0.5,
              fill = "#3B4F8E")+
  geom_line(data = sbc.algae_dmh.predictions, 
            aes(x = x, y = predicted))+
  geom_jitter(aes(x = dmh_cover.mean, y = algae_cover.mean),
             alpha = 0.6,
             size = 2)+
  labs(x = "% cover of dead holdfasts",
...
# Load and clean data
gce.biomass <- read_csv("Data/Georgia Coastal Ecosystems/GCE_Biomass_2000-2024.csv") %>% 
  clean_names()

gce.biomass_dist <- gce.biomass %>% 
  filter(quadrat_area == 0.25,
         total_plant_biomass_m2 != "NA") %>% 
  mutate(plot_disturbance = if_else(plot_disturbance == 1, "Disturbed", "Undisturbed"),
         plot_disturbance = as.factor(plot_disturbance),
         biomass.norm = total_plant_biomass_m2/max(total_plant_biomass_m2)) %>% 
  select(c(1,2,4,7,16,24)) 


# Summarize data for exploratory looks
gce.biomass_dist.summary <- gce.biomass_dist %>% 
  group_by(plot_disturbance) %>% 
...
Model of biomass ~ wrack disturbance 
```{r}

# Check normality of biomass variable
hist(gce.biomass_dist$total_plant_biomass_m2)
hist(log(gce.biomass_dist$total_plant_biomass_m2)) # Use log-transformed biomass variable in mixed model

# Mixed effects model of biomass ~ wrack disturbance,random effects: year;  plot nested within zone nested within site
gce.biomass_dist.glmm <- glmmTMB(
  log(total_plant_biomass_m2 + 0.1) ~ plot_disturbance + (1 | year) + (1 | site/plot),
  data = gce.biomass_dist,
  family = gaussian()
)

summary(gce.biomass_dist.glmm)

# Check residuals
hist(residuals(gce.biomass_dist.glmm))
car::qqPlot(residuals(gce.biomass_dist.glmm))

# Plot model predictions and create dataframe 
plot(ggpredict(gce.biomass_dist.glmm, terms = ~plot_disturbance))
gce.biomass_dist.predictions <- as.data.frame(ggpredict(gce.biomass_dist.glmm, terms = ~plot_disturbance))
...
```{r}
# Raw data
ggplot(gce.biomass_dist.predictions, aes(x = x, y = predicted)) +
  # geom_jitter(data = gce.biomass_dist,
  #            aes(x = plot_disturbance, y = total_plant_biomass_m2),
  #            alpha = 0.6,
  #            width = 0.1) +
  ...
  labs(x = "Wrack disturbance",
       y = Marshgrass~biomass~(g/m^2)) +
  theme_classic(base_size = 12)

```

Model of biomass ~ wrack disturbance, normalized data
```{r}

# Check normality of biomass variable
hist(gce.biomass_dist$biomass.norm)
hist(log(gce.biomass_dist$biomass.norm)) # Use log-transformed biomass variable in mixed model

# Mixed effects model of biomass ~ wrack disturbance,random effects: year;  plot nested within zone nested within site
gce.biomass_dist.glmm.norm <- glmmTMB(
 biomass.norm ~ plot_disturbance + (1 | year) + (1 | site/plot),
  data = gce.biomass_dist,
  family = gaussian()
)

summary(gce.biomass_dist.glmm.norm)

# Check residuals
hist(residuals(gce.biomass_dist.glmm.norm))
car::qqPlot(residuals(gce.biomass_dist.glmm.norm))

# Plot model predictions and create dataframe 
plot(ggpredict(gce.biomass_dist.glmm.norm, terms = ~plot_disturbance))
gce.biomass_dist.predictions.norm <- as.data.frame(ggpredict(gce.biomass_dist.glmm.norm, terms = ~plot_disturbance))

```

Model visualization, normalized data
```{r}
ggplot(gce.biomass_dist.predictions.norm, aes(x = x, y = predicted)) +
  geom_errorbar(aes(ymax = conf.high,
                ymin = conf.low),
                width = 0) +
  geom_point(size = 3) +
  geom_point(data = gce.biomass_dist,
             aes(x = ))
  scale_x_discrete(limits = rev(levels(gce.biomass_dist.predictions.norm$x))) +
  scale_y_continuous(limits = c(0,0.5),
                     expand = c(0,0)) +
  labs(x = "Wrack disturbance",
       y = "Marshgrass biomass (normalized)") +
  theme_classic(base_size = 12)
...
luq.seeding_counts.trim <- luq.mortality_counts %>% 
  filter(treatment == "Trim&clear" | treatment == "Trim+Debris",
         year %in% 2009) %>% 
...
gce.effects <- broom.mixed::tidy(gce.biomass_dist.glmm.norm, effects = "fixed", conf.int = TRUE)


# Assemble effect sizes from each ecosystem type
lter.effects <- rbind(knz.effects, vcr.effects, mcr.effects, bnz.effects, sbc.effects, luq.effects, gce.effects)
lter.effects <- lter.effects %>% 
  filter(term != "(Intercept)") %>% 
  mutate(ecosystem = c("Tallgrass prairie", "Oyster reef", "Coral reef", "Boreal forest", "Kelp forest", "Tropical forest", "Saltmarsh"),
         significance = ifelse(conf.low > 0 | conf.high < 0, "Significant", "Not Significant"))

write_csv(lter.effects, "Data/effect_sizes.csv")
```
